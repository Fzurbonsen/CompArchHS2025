...
- **Program Errors:** Errors that occur when the flash array being read contains errors. And these values are then used to program new data. These stem mainly from tow-step or foggy-fine programming, and copyback operations.
...
Shadow program sequencing tries to ensure that a fully-programmed wordline experiences interference minimally from cell-to-cell ptrogram interference. Shadow program sequencing assigns a unique page number to each page within a block. This allows two-step ptrogramming to write to pages in increasing order inside a block, such that a fully-programmed wordline experiences interference only from the most-signifcant bit(MSB) page programming of the wordline directly above it.

#### Neighbour-Cell Assisted Error Correction

This mitigation technique workys by using the data values stored in the adjacent wordline to determine a better set of read reference voltages for the wordline being read. This leads to a more accurate identifiaction of the logical data value, as the data in the adjacent wordline was partially responsible for shifting the threshold voltage of the cells in the wordline being read.
...
- **In-Place Refresh:** Remapping incurrs a cost due to the additional writes it causes and therefore the additional wearout. In-place refresh incrementally replenshies the lost charge of each page at its current location without remapping.
...
This mitigation allows the read reference voltages to dynamically adjust to changes in distrbutions. This allows to adapt to changing threshold voltage distrubitons.

#### Voltage Optimization

Many raw bit errors in NAND flash memory are affected by the various voltages esed within the memory to enable reading of values. As a result, optimizing these voltages such that they minimize the total number of errors can greatly mitigagte error counts. For this there are multiple approaches.
...
#### SSD Data Recovery
When the afformentioned correction mechanisms fail, then data loss can occur and the SSD is considered to have reached the end of its lifetime. To recover this data from the SSD, the controller can emply *Retention Failure Recovery* and *Read Disturb Recovery*.

### Emerging Reliability Issues for 3D NAND Flash

The paper states that current isses of 3D NAND flash is different from. But the paper states that with increasing scaling of 3D NAND flash memory it will encounter the same issue as planar 2D NAND flash memory. But issues like cell-to-cell interference will worsen as in a 3D layout each cell has more neighbours.

## Strengths
The paper provides a through review of errors in flash-based SSDs as well as the mitigation techniques thereof. The paper also provides a good introduction into the underlying concepts as well as the high level consequences of these issues.
...
## Takeaways
- NAND flash-based SSDs are a very strong technology which comes with its own challanges and error sources. It is necessary to address these error sources to provide functioning computer systems.

- Future technology scaling and 3D stacked NAND flash systems will encounter the same issues and increase the prevelance of these error sources.