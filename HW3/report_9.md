...
## Strengths
The paper provides a strong solution which is limited to DRAM only and does not violate DDR4 protocols or create the need to share proprietary information about the DRAM.
...
The proposed design of Panopticon leaves open space on the DRAM chip which means wasted space in an already tight environment. While the authors reason that this provides space for testing logic, etc. this rather seems like a detriment to the implementation and its viability.

## My POV
While the paper makes a strong case for Panopticon, the two weakneses outlined above are significant. To mitigate the first a sufficient degree of separation in DRAM is needed between the counters and the rest of the DRAM but this in itself worsens the second issue which is the inefficient space usage. But it could be possible that a smart layout might solve these issues.
