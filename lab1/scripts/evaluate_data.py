...
def find_best_instance_for_all_cache_blocks(data, ic_size, dc_size):
    """For each (IC_size, DC_size) pair, find the instance with the smallest Cycles."""
    if not data:
        print("No data available.")
        return []
...
def find_best_instance_for_all_cache_assoc(data, ic_size, dc_size, ic_block, dc_block):
    """For each (IC_size, DC_size) pair, find the instance with the smallest Cycles."""
    if not data:
        print("No data available.")
        return []
...
def main():
    ...
    # plot_cycles_vs_ic_for_max_dc(best_instances)
    # plot_cycles_vs_ic_for_each_dc(best_instances)
    # plot_cycles_vs_dc_for_each_ic(best_instances)

    best_instances_block = find_best_instance_for_all_cache_blocks(data, 4*1024, 64*1024)
    plot_heatmap(best_instances_block, 'IC_block', 'DC_block')

    best_instances_assoc = find_best_instance_for_all_cache_assoc(data, 16*1024, 64*1024, 16, 16)
    plot_heatmap(best_instances_assoc, 'IC_assoc', 'DC_assoc')
